...
public class Enemy : MonoBehaviour {
    ...
    public int points;
    //the lower the dropChance number is, the higher the odds of dropping a power-up are
    ...
    public void GetDamage(int damage) 
    {
     ...
     private void OnDestroy() //on destruction enemies give score and have a chance of dropping power-ups
     {
        if(health <= 0) {
            OnScoreUpdate?.Invoke(points);//points earned
            rn = UnityEngine.Random.Range(0, dropChance);//power-up drops

            if(rn == 0) Instantiate(drop, transform.position, drop.transform.rotation);
        }
    }
